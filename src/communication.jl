...
providing enhanced error handling and async support.
...
function Base.run(win::Window, code::AbstractString)
    win.exists || throw(WindowClosedError(win.id, "run"))
...
function send_message_to_julia(win::Window, message)
    win.exists || return  # Silently ignore if window is closed
...
function wait_for_message(win::Window; timeout::Union{Real,Nothing} = nothing)
    win.exists || throw(WindowClosedError(win.id, "wait_for_message"))
...
function on_message(callback::Function, win::Window; async::Bool = true)
    win.exists || throw(WindowClosedError(win.id, "on_message"))

    if async
        @async begin
            try
                while win.exists && isopen(win.msg_channel)
                    msg = take!(win.msg_channel)
                    @async callback(msg)  # Handle each message asynchronously
                end
            catch e
                if win.exists  # Only log if window is still supposed to be active

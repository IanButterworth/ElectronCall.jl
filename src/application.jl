...
function Application(
    ...
    app =
        Application(connection, proc, secure_cookie, Window[], true, security_config, name)
...
- `electron_args::Vector{String} = String[]`: Additional Electron arguments
...
app = Application(
    ...
    electron_args = ["--enable-logging", "--v=1"]
)
```
"""
function Application(;
    ...
    append!(electron_cmd_args, electron_args)
...
function validate_security_config(config::SecurityConfig)
    ...
    if config.node_integration && config.sandbox
        throw(
            SecurityError(
                "Cannot enable node_integration with sandbox mode",
                "configuration",
            ),
        )
    end

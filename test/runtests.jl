...
""",
    ...
    @testset "Security Configuration Edge Cases" begin
        ...
        @test length(custom_config.allowed_origins) == 2
        @test custom_config.csp_policy == "default-src 'self'"

        # Test SecurityConfig with no CSP policy
        minimal_config = SecurityConfig(csp_policy = nothing, allowed_origins = String[])
        @test minimal_config.csp_policy === nothing
        @test isempty(minimal_config.allowed_origins)

        # Test SecurityConfig show/validation behavior
        @test minimal_config.context_isolation == true  # Default should be secure
        @test minimal_config.sandbox == true  # Default should be secure
        @test minimal_config.web_security == true  # Default should be secure
    end

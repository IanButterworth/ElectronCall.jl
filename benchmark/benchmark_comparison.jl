...
function benchmark_electroncall(config::BenchmarkConfig)
    ...
    for i = 1:config.iterations
        ...
        try
            ...
            _, elapsed = @time_it begin
                tasks = []
                for task_id = 1:config.concurrent_tasks
...
function compare_results(electron_results, electroncall_results)
    ...
    function compare_metric(name, electron_vals, electroncall_vals, lower_is_better = true)
        ...
        println(
            @sprintf("%-20s %s %s wins by %.1f%%", name, symbol, winner, abs(improvement))
        )
    ...
    compare_metric(
        "Startup Time:",
        electron_results.startup_time,
        electroncall_results.startup_time,
    )
    compare_metric(
        "Window Creation:",
        electron_results.window_creation_time,
        electroncall_results.window_creation_time,
    )
    compare_metric(
        "JS Execution:",
        electron_results.js_execution_time,
        electroncall_results.js_execution_time,
    )
    compare_metric(
        "Cleanup Time:",
        electron_results.cleanup_time,
        electroncall_results.cleanup_time,
    )

    # Throughput comparison (higher is better)
    if !isempty(electron_results.js_throughput) &&
       !isempty(electroncall_results.js_throughput)
        electron_throughput = mean(electron_results.js_throughput)
        electroncall_throughput = mean(electroncall_results.js_throughput)
        improvement =
            (electroncall_throughput - electron_throughput) / electron_throughput * 100
        winner = improvement > 0 ? "ElectronCall.jl" : "Electron.jl"
        symbol = improvement > 0 ? "🚀" : "⚠️"
